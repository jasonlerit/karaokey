...
Embed the supported YouTube player on the host display and provide authorized play, pause, restart, skip, and volume controls while reconciling detectable player events with authoritative room state.
...
- Provide play, pause, restart, skip, and volume controls on the host screen.
...
2. A valid host can play, pause, restart, skip, and adjust local player volume.
...
- Confirm that volume is local to the host player and is not synchronized to guest clients.
